<?php
	class Chart extends DBconn{
		...
		public function getBrandName($table_name="",$brand_id="")
		{
			$sql = "SELECT `brand_name` FROM " . $table_name . " WHERE `brand_id` = " . $brand_id;
		...
		public function viewBrandTickets($brand_id,$cond="")
		{
			if($cond=="")
			{
				...
				$sql = "SELECT COUNT( ticket_id ) as count , STATUS FROM  `nipl_ticket`  WHERE dept_id=" . $_SESSION['dept_id'] ." and brand_id = " . $brand_id . " GROUP BY STATUS";
			}
			else
			{
				//$sql = "SELECT COUNT( ticket_id ) as count , STATUS FROM  `nipl_ticket`  WHERE dept_id=" . $_SESSION['dept_id'] ." and brand_id = " . $brand_id . " ".$cond." GROUP BY STATUS";
		...
		{
			//$sql = "SELECT count(ticket.tonality_id) as tonalitycount,tonality_type FROM nipl_ticket ticket LEFT JOIN nipl_tonality tonality ON ticket.tonality_id=tonality.tonality_id LEFT JOIN nipl_department department ON ticket.dept_id=department.dept_id WHERE department.dept_id=" . $dept_id . " " .$cond. " GROUP BY(ticket.tonality_id) ";
			$sql = "SELECT count(tonality_id) AS tonalitycount,tonality_type FROM (SELECT ticket.tonality_id,tonality_type FROM nipl_ticket ticket LEFT JOIN nipl_tonality tonality ON ticket.tonality_id=tonality.tonality_id LEFT JOIN nipl_department department ON ticket.dept_id=department.dept_id WHERE department.dept_id=" . $dept_id . " " . $cond . " UNION ALL SELECT ticket.tonality_id,tonality_type FROM nipl_tonalitycsv ticket LEFT JOIN nipl_tonality tonality ON ticket.tonality_id=tonality.tonality_id LEFT JOIN nipl_department department ON ticket.dept_id=department.dept_id WHERE department.dept_id=" . $dept_id. " " . $cond . ") AS table1 GROUP BY tonality_type";
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
		}
		...
		public function viewCloseInbetween_brand($date,$brand_id)
		{
		...
		public function viewClasification($dept_id,$cond)
		{
		...
		{
			$sql = "SELECT SUM(DATEDIFF( updated_date, created_date )) AS sum,COUNT(*) AS count FROM nipl_ticket WHERE status=2 AND date(updated_date) >= '" . $date2 . "' AND date(updated_date) <= '" . $date1 . "' AND dept_id=" . $_SESSION['dept_id'];
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
		}
		
		public function viewBrandTatheader($date1,$date2)
		{
		...
		public function viewTicketmonth($date,$query_staffid)
		{
			$sql = "SELECT count(*) AS count FROM nipl_ticket WHERE dept_id='" . $query_staffid . "' AND created_date LIKE '" . $date . "%'";
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
		}
		
		public function viewStillopenmonth($date,$query_staffid)
		{
			$sql = "SELECT count(*) AS count FROM nipl_ticket WHERE dept_id='" . $query_staffid . "' AND created_date LIKE '" . $date . "%'  AND status='0'";
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
		}
		
		public function viewEscalatednmonth($date,$query_staffid)
		{
			$sql = "SELECT count(*) AS count FROM nipl_ticket WHERE dept_id='" . $query_staffid . "' AND created_date LIKE '" . $date . "%'  AND status='1'";
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
		}
		
		public function viewClosednmonth($date,$query_staffid)
		{
			$sql = "SELECT count(*) AS count FROM nipl_ticket WHERE dept_id='" . $query_staffid . "' AND created_date LIKE '" . $date . "%'  AND status='2'";
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
		}
		
		public function viewTatmonth($date1,$date2,$query_staffid)
		{
	
			$sql = "SELECT SUM(DATEDIFF( updated_date, created_date )) AS sum,COUNT(*) AS count FROM nipl_ticket WHERE dept_id='" . $query_staffid . "' AND created_date LIKE '" . $date1 . "%' AND date(updated_date) <= '" . $date2 . "' AND status='2'";
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
		...
		{
			$sql = "SELECT DATEDIFF( updated_date, created_date ) AS hours FROM nipl_ticket WHERE status=2 AND dept_id=" . $_SESSION['dept_id'] . $cond;
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
		}
		
		public function viewOpendash($date)
		{
			$sql = "SELECT count(ticket_id) as opencount FROM nipl_ticket WHERE created_date LIKE '%".$date."%' AND status = 0";
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
		...
		public function viewClosedash($date)
		{
			$sql = "SELECT count(ticket_id) as closecount FROM nipl_ticket WHERE created_date LIKE '%". $date ."%'  AND status = 2";
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
		...
		public function viewReasonchartdash($cond)
		{
			$sql = "SELECT COUNT( ticket.ticket_id ) AS count, levelone_name, DATE( ticket.created_date ) as created_date FROM  `nipl_ticket` AS ticket LEFT JOIN nipl_levelone AS levelone ON ticket.levelone_id = levelone.levelone_id " . $cond . " GROUP BY ticket.created_date, ticket.levelone_id order by created_date DESC" ;
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
		}
		
		public function viewTatmonthbrand($date1,$date2,$brand_id)
		{
	
			$sql = "SELECT SUM(DATEDIFF( updated_date, created_date )) AS sum,COUNT(*) AS count FROM nipl_ticket WHERE brand_id='" . $brand_id . "' AND created_date LIKE '" . $date1 . "%' AND date(updated_date) <= '" . $date2 . "' AND status='2'";
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
			
		}
		
		public function viewTonalityDetailsbrand($brand_id,$cond="")
		{
			//$sql = "SELECT count(ticket.tonality_id) as tonalitycount,tonality_type FROM nipl_ticket ticket LEFT JOIN nipl_tonality tonality ON ticket.tonality_id=tonality.tonality_id LEFT JOIN nipl_brands brand ON ticket.brand_id=brand.brand_id WHERE brand.brand_id=" . $brand_id . " " .$cond. " GROUP BY(ticket.tonality_id) ";
			$sql = "SELECT count(tonality_id) AS tonalitycount,tonality_type FROM (SELECT ticket.tonality_id,tonality_type FROM nipl_ticket ticket LEFT JOIN nipl_tonality tonality ON ticket.tonality_id=tonality.tonality_id LEFT JOIN nipl_brands brands ON ticket.brand_id=brands.brand_id WHERE brands.brand_id=" . $brand_id. " " . $cond . " UNION ALL SELECT ticket.tonality_id,tonality_type FROM nipl_tonalitycsv ticket LEFT JOIN nipl_tonality tonality ON ticket.tonality_id=tonality.tonality_id LEFT JOIN nipl_brands brands ON ticket.brand_id=brands.brand_id WHERE brands.brand_id=" . $brand_id . " " . $cond . ") AS table1 GROUP BY tonality_type";
			$result = $this->sql_fetchrowset($this->sql_query($sql));
			return $result;
		}
	}
