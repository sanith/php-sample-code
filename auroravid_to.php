...
//$imapuser         = 'nitin.nallewar@netscribes.com';
//$imappass         = 'Nets@1083';
...
//foreach($emails as $mail) {
	if($emailcount >= 1)
	{
		...
		for($i=0;$i<=$emailcount;$i++)
		{
			...
			echo $sitename  = strpos($subject1, 'httpwww.auroravidto');
			if($sitename != "")
			{
				$site_name = "http://www.auroravid.to";
				...
				$curl_connection = curl_init('http://www.auroravid.to/contact.php');
				...
				echo $position = strpos($result, 'Yourmessagehasbeensent');
				if($position != "")
				{
